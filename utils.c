...
#ifdef _WIN32
    // When running as a screensaver/preview, there's usually no
    // console attached, so the fprintf() above goes nowhere anyone can see.
    // A message box makes sure the failure is actually visible.
    char msgBuf[1024];
    va_start(args, message);
    vsnprintf(msgBuf, sizeof(msgBuf), message, args);
    va_end(args);
    // MB_TOPMOST + MB_SETFOREGROUND: launched from a desktop/start-menu
    // shortcut, this process owns no visible window for the box to sit on
    // top of, so without these flags it can silently pop up behind
    // Explorer or whatever else has focus -- looking exactly like "nothing
    // happened" when something actually did.
    MessageBoxA(NULL, msgBuf, "Johnny Castaway - Erro fatal",
...
static time_t getVirtualTime()
{
    struct timeval tv;

    gettimeofday(&tv, NULL);
    return tv.tv_sec + ((time_t) gDateOffsetDays * 86400);
}


int getDayOfYear()
{
    time_t t = getVirtualTime();
